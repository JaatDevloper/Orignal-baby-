...
async def show_final_results(context: ContextTypes.DEFAULT_TYPE):
    ...
    for uid, stats in user_data.items():
        leaderboard.append({
            "name": stats.get("name", f"User {uid}"),
            "correct": stats.get("correct", 0),
            "total": stats.get("total", 0),
            "points": stats.get("points", 0)
        })

    leaderboard.sort(key=lambda x: (-x["points"], -x["correct"]))

    message = "🏁 *Quiz Marathon Results* 🏁\n\n"
    for idx, entry in enumerate(leaderboard[:10], 1):
        message += f"{idx}. {entry['name']} – {entry['correct']} correct – {entry['points']:.1f} pts\n"
